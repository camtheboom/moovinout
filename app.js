...
function popupHtml(s){
  ...
  const n = escapeHtml(s.n);
  return `<span class="pn">${n}</span><br>
    ...
    <div class="popactions">
      <button class="setdest" data-act="setdest" data-lat="${s.lat}" data-lon="${s.lon}" data-name="${n}">Set as destination</button>
      <button class="setdest" data-act="savespot" data-lat="${s.lat}" data-lon="${s.lon}" data-name="${n}">Save spot</button>
    </div>`;
}
...
// Ids are generated by uid(), so anything that isn't [a-z0-9] never came from us.
function safeId(v){
  const clean = String(v == null ? "" : v).replace(/[^a-z0-9]/gi, "").slice(0, 32);
  return clean || uid();
}
...
function loadSaved(){
  try{
    ...
    return list
      ...
      // The id is re-derived rather than trusted: it ends up in a data attribute, and
      // localStorage is editable by anything that has ever run on this origin.
      .map(s => ({id: safeId(s.id), label: String(s.label || "Saved spot"), lat: s.lat, lon: s.lon}));
  }catch(err){
...
// Popup buttons carry their arguments in data-* attributes and are dispatched by the
// delegated handler below, rather than through inline onclick. Two reasons: an inline
// handler means interpolating data into a JS string inside an HTML attribute — two
// escaping layers that are easy to get subtly wrong — and it forces script-src
// 'unsafe-inline' into the CSP, which defeats most of the point of having one.
function destFromSaved(id){
  const s = savedSpots.find(x => x.id === id);
  if(s) setDestination(s.lat, s.lon, s.label);
}

// Popups live inside the map container and are created and destroyed constantly, so
// the listener goes on the document once instead of on each button.
document.addEventListener("click", e => {
  ...
  switch(btn.dataset.act){
    case "setdest":
      if(Number.isFinite(lat) && Number.isFinite(lon)) setDestination(lat, lon, btn.dataset.name || "");
      break;
    case "savespot":
      if(Number.isFinite(lat) && Number.isFinite(lon)) saveSpot(lat, lon, btn.dataset.name || "");
      break;
    case "destsaved": destFromSaved(btn.dataset.id); break;
    case "removespot": removeSpot(btn.dataset.id); break;
  }
});
...
function savedPopupHtml(spot){
  ...
  return `<span class="pn">${escapeHtml(spot.label)}</span><br>${body}
    <div class="popactions">
      <button class="setdest" data-act="destsaved" data-id="${escapeHtml(spot.id)}">Set as destination</button>
      <button class="setdest" data-act="removespot" data-id="${escapeHtml(spot.id)}">Remove</button>
    </div>`;
}
...
map.on("click", e => {
  ...
  L.popup().setLatLng(e.latlng).setContent(
    `<span class="pn">≈ ${est.total} min to ${escapeHtml(dest.name)}</span><br>
     ...
     <div class="popactions">
       <button class="setdest" data-act="savespot" data-lat="${e.latlng.lat}" data-lon="${e.latlng.lng}">Save this spot</button>
     </div>`
  ).openOn(map);
});
...
function applyShareUrl(){
  ...
  const cityChanged = !!(wanted && wanted !== city);
  if(cityChanged) applyCity(wanted);
...
window.addEventListener("hashchange", () => {
  ...
  const cityChanged = applyShareUrl();
  renderLegend(); renderStations(); renderSaved(); refreshLive();
  // Rent figures are per-state, so a link that crosses cities has to invalidate them —
  // setCity() does this, but a shared link goes through applyCity() directly and used to
  // leave NSW bond markers sitting on a Melbourne map.
  if(cityChanged && typeof onCityChanged === "function") onCityChanged(city);
});
